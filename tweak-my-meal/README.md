...
All AI agent prompts are centralized for easy modification:

### Backend Prompts (Python)

**File:** [`backend/app/agents/prompts.py`](backend/app/agents/prompts.py)

| Prompt | Purpose |
|--------|---------|
| `VISION_AGENT_SYSTEM` | Analyzes food images, classifies as meal/ingredients |
| `MEAL_UNDERSTANDING_SYSTEM` | Normalizes user input into structured format |
| `SUGGESTION_AGENT_SYSTEM` | Generates healthier meal alternatives |
| `RECIPE_AGENT_SYSTEM` | Creates full recipes with ingredients and steps |
| `MEMORY_UPDATE_SYSTEM` | Learns from user feedback, updates preferences |

### Flutter Prompts (Dart)

**File:** [`lib/services/agent_prompts.dart`](lib/services/agent_prompts.dart)

| Prompt | Purpose |
|--------|---------|
| `critic` | Analyzes meal nutritional value |
| `chef` | Creates healthier meal variations |
| `planner` | Generates daily meal plans |
| `interviewerInit` | Onboarding conversation starter |
| `profiler` | Extracts user profile from conversation |

### Customizing Prompts

To modify AI behavior:

1. **Backend agents**: Edit `backend/app/agents/prompts.py`
2. **Flutter onboarding**: Edit `lib/services/agent_prompts.dart`
3. Restart the respective service after changes
